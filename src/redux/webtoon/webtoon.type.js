export const MAIN_WEBTOONS = 'webtoon/MAIN_WEBTOONS';

export const SET_GENRES = 'webtoon/SET_GENRES';
export const SET_GENRE = 'webtoon/SET_GENRE';

export const SET_PAGE = 'webtoon/SET_PAGE';
export const SET_TOTAL_PAGE = 'webtoon/SET_TOTAL_PAGE';

export const SET_CATEGORIES = 'webtoon/SET_CATEGORIES';
export const SET_ACTIVE_CATEGORIES = 'webtoon/SET_ACTIVE_CATEGORIES';

export const SET_WEBTOON = 'webtoon/SET_WEBTOON';
export const SET_EPISODE = 'webtoon/SET_EPISODE';
export const SET_VIEWER = 'webtoon/SET_VIEWER';
